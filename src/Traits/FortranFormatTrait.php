        ...
        switch ($format_type) {
            ...
            case 'F':
                ...
                if ($explode__format_value[1] > 0) {
                    ...
                    $return .= $fraction;
                } elseif ($explode__format_value[1] == 0) {
                    ...
                    if (intval($value) == 0 && intval($fractional_rounded_by_format_value) == 0) {
                        $kk = substr($kk, 1);
                    } elseif (strlen(intval($k)) > $f) { /*  Il numero arrotondato e' maggiore del numero richeisto; se ho 999.9123 a F3.0, non posso scrivere 1000 e quindi errore */
                        return str_pad(
                            '',
                            $f,
                            '*',
                ...
                if ($value < 0) {
                    //$return = strrev(preg_replace(strrev('/=/'), strrev('-'), strrev($return), 1));
                    $p = quotemeta($str_pad_string);
                    $return = strrev(preg_replace("/${p}/", '-', strrev($return), 1));
                }
                ...
                if ($float_format == 'old') {
                    ...
                    // La prima cosa che faccio e' sostituire '-0.44' con '-.44' e risparmio un carattere
                    if (strlen($valueNew) > $explode__format_value[0] && $value < 0) {
                        $valueNew = str_replace('-0', '-', $valueNew);
                    }
